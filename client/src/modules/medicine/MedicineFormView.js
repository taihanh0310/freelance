...
import {loadOnlyMedicineGroup as medicineGroupLoadList} from 'modules/medicineGroup/actions/list';
...
class MedicineFormView extends React.Component{
	constructor(){
		super();
		this.state = {
                    confirm: false,
	...
	_onValidationSubmit(){
		...
		this._onChangeField('retail_price', retail_price);
                this._onChangeField('medicine_barcode', medicine_barcode);
			  							...
			  							<tbody id="list-container">
			  								{
												this.props.medicines.list.map((l, key) => {
													if(l.id !== -1){
														let header = null;
														if(key === 0 || l.header === 0){
															const group = (typeof l.group !== 'undefined') ? l.group : {};
															header = <td rowSpan={l.size}>{group.name}</td>;
														}
														...
														return (
															<tr key={l.id} onClick={this._onClickRow.bind(this, l, key)} id={`row-${key}`}>
																...
																<td className={selectedMedicine}></td>
																<td className={selectedMedicine + ' text-right'}>{Numeral(l.input_price).format('0,0')}</td>
																<td className={selectedMedicine + ' text-right'}>{Numeral(l.wholesale_price).format('0,0')}</td>
																<td className={selectedMedicine + ' text-right'}>{Numeral(l.retail_price).format('0,0')}</td>
																<td className={selectedMedicine + ' text-right'}>{Numeral(l.listed_price).format('0,0')}</td>
																<td className={selectedMedicine + ' text-center'}>
																...
																<td className={selectedMedicine + ' text-right'}>{l.max_number}</td>
																<td className={selectedMedicine + ' text-right'}>{l.min_number}</td>
															</tr>
														);
													}else{
					  									if(this.props.medicines.list.length === 1){
			  							...
			  							<div className="row">
			  								...
			  								<div className="col-md-4">
			  									<div className={this.props.medicineForm.errors.coefficient?'form-group has-error':'form-group'}>
			  										...
			  										<div>
			  											<input type="number" min="0" className="form-control text-right" ref="coefficient"
			  							...
			  							<div className="row">
			  								...
			  								<div className="col-md-4">
			  									<div className={this.props.medicineForm.errors.max_number?'form-group has-error':'form-group'}>
			  										<label className="control-label4">
...
module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(MedicineFormView);