...
const INITIAL_STATE = {listRoot: [], list: [], load: false, search: {name: '', prescription_drug: '', medicine_group_id: ''}};

const Reducer = (state = INITIAL_STATE, action) => {
	switch(action.type){
		case MEDICINE_CHANGE_LIST:
			return {...state, listRoot: action.payload.listRoot, list: action.payload.list};
		case MEDICINE_PUSH_LIST:
			return {...state, listRoot: [...state.listRoot, action.payload], list: [...state.list, action.payload]};
		case MEDICINE_REMOVE_LIST:
