...
const Reducer = (state = INITIAL_STATE, action) => {
	switch(action.type){
		...
		case HOA_DON_NHAP_XUAT_FORM_CHILD_CHANGE:
			return {...state, values: {...state.values, [action.payload.field]: action.payload.value}};
		case HOA_DON_NHAP_XUAT_FORM_CHILD_CHANGE_MODE:
			return {...state, mode: action.payload};
		case HOA_DON_NHAP_XUAT_FORM_CHILD_CLEAR:
			return {...state, ...INITIAL_STATE, mode: state.mode};
		case HOA_DON_NHAP_XUAT_FORM_CHILD_VALIDATION:
