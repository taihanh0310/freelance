...
import * as InternalStockDeliveryFormChildActions from 'modules/internalStockDelivery/actions/formChild';
import {formChange as internalStockDeliveryFormChange} from 'modules/internalStockDelivery/actions/form';

class InternalStockDeliveryChildForm extends React.Component {
    constructor(){
        super();
        this.state = {
            shipments: [], 
    ...
    _bindChangeMedicine(event){
        ...
        .then(response => {
            ...
            axios.get(`${Config.API_URL}pharmacy-warehouse/${this.props.internalStockDeliveryForm.values.from_drug_store_id}/medicine/${value}`)
            .then(response => {
    ...
    _bindChangeShipment(event){
        ...
        axios.get(`${Config.API_URL}pharmacy-warehouse/${this.props.internalStockDeliveryForm.values.from_drug_store_id}/medicine/${this.props.internalStockDeliveryFormChild.values.medicine_id}/shipment/${value}`)
        .then(response => {
            const shipmentObj = response.data.data[0];
            this.props.formChange('inventory_number', shipmentObj.quantity);
            this.props.formChange('medicine_created_date', shipmentObj.medicine_created_date);
            this.props.formChange('medicine_limited_date', shipmentObj.medicine_limited_date);
            this.props.formChange('country_id', shipmentObj.country_id);
            this.props.formChange('country_name', shipmentObj.country_name);
        });
    }

    // Ghi nhan
    _bindAccept(){
        if(this.props.internalStockDeliveryFormChild.mode === 'add'){
            this._onValidationSubmit()
            .then(valid => {
                    if(valid){
                        const selectedDetail = this.props.internalStockDeliveryFormChild.values;
                        const listChild = this.props.internalStockDeliveries.listChild.list;
        ...
        }else if(this.props.internalStockDeliveryFormChild.mode === 'edit'){
            
            // const total_money_before_discount = this.props.internalStockDeliveries.selectedDetail.total_money_before_discount;
            const removeObj = this.props.internalStockDeliveries.listChild.list[this.state.keyRowSelected];
            const discount_remove = removeObj.total_price;
            const list_remain = this._removeByKey(this.props.internalStockDeliveries.listChild.list, this.state.keyRowSelected);
            setTimeout(() => {
...
//                this.props.formClear(); 
    ...
    _calculatorMoneyParentForm(money_add, mode){
        ...
        var delivery_total_money = this.props.internalStockDeliveryForm.values.delivery_total_money_added;
        var total_discount_money = this.props.internalStockDeliveryForm.values.total_discount_money;
        var discount_rate = this.props.internalStockDeliveryForm.values.discount_rate;
        ...
        this.props.internalStockDeliveryFormChange('total_money_before_discount', total_money_before_discount_added);
        this.props.internalStockDeliveryFormChange('delivery_total_money', delivery_total_money_added);
        this.props.internalStockDeliveryFormChange('total_discount_money', total_discount_money_added);
    }

    _bindRemoveChild(child, key){
        // const total_money_before_discount = this.props.internalStockDeliveries.selectedDetail.total_money_before_discount;
        const removeObj = this.props.internalStockDeliveries.listChild.list[key];
        const discount_remove = removeObj.total_price;
        const list_remain = this._removeByKey(this.props.internalStockDeliveries.listChild.list, key);
        setTimeout(() => {
            this.props.changeListChild(list_remain);
            this.props.clearListChildSelectedDetail();
            this._calculatorMoneyParentForm(discount_remove, 'delete');
    ...
    _onChangeChildField(field, value){
        ...
        switch(field){
            case 'quantity': {
                ...
                else if(parseFloat(value) > parseFloat(this.props.internalStockDeliveryFormChild.values.inventory_number)) 
                    error = 'application.validation.max';
                else
                    error = '';
                    let totalPrice = parseFloat(value) * parseFloat(this.props.internalStockDeliveryFormChild.values.input_price);
    ...
    _onValidationSubmit(){
        const {medicine_id, shipment_no, quantity, inventory_number} = this.props.internalStockDeliveryFormChild.values;
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const errors = this.props.internalStockDeliveryFormChild.errors;
                let valid = true;
                            ...
                            <div className="form-body">
                                <div className="row">
                                    <div className="col-md-3">
                                        <div className={this.props.internalStockDeliveryFormChild.errors.medicine_id?'form-group has-error':'form-group'}>
                                            ...
                                            <span className="help-block" style={{display: this.props.internalStockDeliveryFormChild.errors.medicine_id ? 'block': 'none'}}>
                                                <Translate value={this.props.internalStockDeliveryFormChild.errors.medicine_id}/>
                                            </span>
                                        </div>
                                    </div>
                                    <div className="col-md-3">
                                        <div className={this.props.internalStockDeliveryFormChild.errors.shipment_no?'form-group has-error':'form-group'}>
                                            <label>Số lô</label>
                                            ...
                                            <span className="help-block" style={{display: this.props.internalStockDeliveryFormChild.errors.shipment_no ? 'block': 'none'}}>
                                                <Translate value={this.props.internalStockDeliveryFormChild.errors.shipment_no}/>
                                            </span>
                                        </div>
                                    </div>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <option value=""></option>
                                            {
                                                this.state.shipments.map((shipment, key) => {
                                                    return (
                                                        <option key={key} value={shipment.shipment_no}>
                                                            {Numeral(shipment.quantity).format('0,0')}
                                                        </option>
                                                    )
                                                })
                                            }
                                ...
                                <div className="col-md-3">
                                    <div className={this.props.internalStockDeliveryFormChild.errors.quantity ?'form-group has-error':'form-group'}>
                                        ...
                                        <span className="help-block" style={{display: this.props.internalStockDeliveryFormChild.errors.quantity ? 'block': 'none'}}>
                                                <Translate value={this.props.internalStockDeliveryFormChild.errors.quantity}/>
                                            </span>
                                    </div>
                                </div>
                                <div className="col-md-3">
                            ...
                            <div className="row">
                                <div className="table-scrollable">
                                    <table className="table fixed-header table-bordered" id="search_bill_result_list">
                                        ...
                                        {
                                            (this.props.internalStockDeliveries.listChild === 'undefined' || this.props.internalStockDeliveries.listChild.list === 'undefined' || this.props.internalStockDeliveries.listChild.list.length == 0)
                                            ?
                                            (    <tr>
                                            ...
                                            :
                                            (
                                            this.props.internalStockDeliveries.listChild.list.map((child, key) => {
                                                const selected = (this.props.internalStockDeliveries.listChild.selectedDetail.id === child.id) ? 'row-active': '';
                                                return (
                                                    <tr key={key} onClick={this._onClickListChild.bind(this, child, key)} className={selected}>
                                                        ...
                                                        <td className="text-right">{Numeral(child.inventory_number).format('0,0')}</td>
...
const mapDispatchToProps = (dispatch) => {
    return Redux.bindActionCreators({
        ...
        ...InternalStockDeliveryFormChildActions,
        internalStockDeliveryFormChange
    }, dispatch);
};

module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(InternalStockDeliveryChildForm);