...
class InternalStockDeliveryParentSearchResult extends React.Component {
    constructor(){
        super();
        this.state = {
    ...
    _onClickRow(internalStockDelivery, key){
        this.props.chooseSelectedDetail(internalStockDelivery);
        this.props.formChangeMode('edit');
        setTimeout(() => {
            axios.get(`${Config.API_URL}pharmacy-warehouse/${internalStockDelivery.from_drug_store_id}/medicine`)
            .then(response => {
                ...
                <div className="portlet box green">
                    ...
                    <div className="table-scrollable">
                        <table className="table fixed-header table-bordered" id="search_bill_result_list">
                            ...
                            {
                                (this.props.internalStockDeliveries === 'undefined' || this.props.internalStockDeliveries.list === 'undefined' || this.props.internalStockDeliveries.list.length == 0)
                                ?
                                (    <tr>
                                ...
                                :
                                (
                                    this.props.internalStockDeliveries.list.map((internalStockDelivery, key) => {
                                        const selected = (this.props.internalStockDeliveries.selectedDetail.id === internalStockDelivery.id) ? 'row-active': '';
                                        return (
                                            <tr key={key} className={selected} onClick={this._onClickRow.bind(this, internalStockDelivery, key)}>
                                                <td>{internalStockDelivery.code}</td>
                                                <td>{Helper.formatStringToDate(internalStockDelivery.delivery_date)}</td>
                                            </tr>
                                        );
                                    })
                                )
...
module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(InternalStockDeliveryParentSearchResult);