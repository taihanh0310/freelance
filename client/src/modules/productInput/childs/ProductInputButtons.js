...
class ProductInputButtons extends React.Component {
    constructor() {
        super();
        this.state = {
    ...
    _onValidationSubmit() {
        const {
            ...
            supplier_id
        } = this.props.productInputForm.values;
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const errors = this.props.productInputForm.errors;
                let valid = true;
    ...
    _onSave() {
        ...
        const data = this.props.productInputForm.values;
        // Get form data detail
        const details = this.props.productInputs.listChild.list;

        switch (this.props.productInputForm.mode) {
            case 'add':
            {
    ...
    _onDelete() {
        let id = this.props.productInputs.selectedDetail.id;
        if (typeof id === "undefined" || id == -1) {
            Helper.warningAlert(<Translate value="application.text.chooseItem"/>);
        } else {
            this.props.formDelete(this.props.productInputs.selectedDetail.id)
                    .then(() => {
            ...
            axios.post(`${Config.API_URL}product-input/pdfExport`, {id}, {
                responseType: 'arraybuffer'
                    ...
                    .then((response) => {
                        ...
                        const fileName = `phieu_nhap_kho_${date}.pdf`;
...
module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(ProductInputButtons);
