...
class Buttons extends React.Component{
    ...
    _onPrintReview(){
        let values = $.extend({}, this.props.soNhapThuocs.search);
              
        axios.post(`${Config.API_URL}so-nhap-thuoc/pdfExport`, {values}, {
        ...
        .then((response) => {
            ...
            const fileName = `so_nhap_thuoc_${date}.pdf`;
    ...
    _onExcelPrintReview(){
        ...
        axios.post(`${Config.API_URL}so-nhap-thuoc/excelExport`, {values}, {
        responseType: 'arraybuffer'
        })
        .then((response) => {
            let downloadLink = document.createElement('a');
            ...
            const date = Moment(new Date()).format('YYYY.MM.DD_HH.mm.ss');
            const fileName = `so_nhap_thuoc_${date}.xlsx`;
            downloadLink.download = fileName;
