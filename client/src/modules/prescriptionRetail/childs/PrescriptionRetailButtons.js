...
class PrescriptionRetailButtons extends React.Component{
        constructor(){
            super();
            this.state = {
        ...
        _onValidationSubmit(){
            ...
            } = this.props.prescriptionRetailForm.values;
            ...
            return new Promise((resolve, reject) => {
                setTimeout(() => {
                    const errors = this.props.prescriptionRetailForm.errors;
                    let valid = true;
        ...
        _onSave(){
            ...
            const data = this.props.prescriptionRetailForm.values;
            // Get form data detail
            const details = this.props.prescriptionRetails.listChild.list;

            switch(this.props.prescriptionRetailForm.mode){
                case 'add':{
                    this._onValidationSubmit()
                    .then(valid => {
                        if(valid){
                            axios.post(`${Config.API_URL}retailer`, {data, details})
                            .then(response => {
                ...
                case 'edit': {
                    ...
                    .then(valid => {
                        if(valid){
                            axios.put(`${Config.API_URL}retailer/${data.id}`, {data, details})
                            .then(response => {
        ...
        _onDelete(){
            ...
            this.props.formDelete(this.props.prescriptionRetails.selectedDetail.id)
            .then(() => {
        ...
        _onPrintReview(){
            let id = this.props.prescriptionRetails.selectedDetail.id;
            
            if (typeof id === "undefined" || id == -1) {
                Helper.alertSelectInvoiceBeforePrint();
            }else{   
                axios.post(`${Config.API_URL}retailer/pdfExport`, {id}, {
                ...
                .then((response) => {
                    ...
                    const fileName = `hoa_don_ban_le_khong_theo_don_${date}.pdf`;
...
module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(PrescriptionRetailButtons);
