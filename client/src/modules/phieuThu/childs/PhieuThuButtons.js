...
class PhieuThuButtons extends React.Component{
    ...
    _onValidationSubmit(){
        const {
            ...
            supplier_id,
            total_money
        } = this.props.phieuThuForm.values;
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const errors = this.props.phieuThuForm.errors;
                let valid = true;
    ...
    _onSave(){
        const values = this.props.phieuThuForm.values;
        switch(this.props.phieuThuForm.mode){
            case 'add':{
    ...
    _onDelete(){
        let id = this.props.phieuThus.selectedDetail.id;
    ...
    _onPrintReview(){
        ...
        if (typeof id === "undefined" || id == -1) {
            ...
            .then((response) => {
                ...
                const fileName = `phieu_thu_chi_tiet_${date}.pdf`;
    ...
    _onPrintList(){
        let values = $.extend({}, this.props.phieuThus.search);
        ...
        .then((response) => {
            ...
            const date = Moment(new Date()).format('YYYY.MM.DD_HH.mm.ss');
            const fileName = `bao_cao_thong_ke_phieu_thu_${date}.pdf`;
            downloadLink.download = fileName;
...
module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(PhieuThuButtons);
