...
import * as KhachLeTraHangFormChildActions from 'modules/khachLeTraHang/actions/formChild';
import {formChange as khachLeTraHangFormChange} from 'modules/khachLeTraHang/actions/form';
...
class ChildForm extends React.Component{
    ...
    _onClickListChild(child, key){
        ...
        axios.get(`${Config.API_URL}khach-le-tra-hang/code/${this.props.khachLeTraHangForm.values.code}/medicine/${child.medicine_id}/shipment_no/${child.shipment_no}`)
        .then(response => {
    ...
    // Ghi nhan
    _bindAccept(){
        if(this.props.khachLeTraHangFormChild.mode === 'add'){
            this._onValidationSubmit()
            .then(valid => {
                    if(valid){
                        const selectedDetail = this.props.khachLeTraHangFormChild.values;
                        const listChild = this.props.khachLeTraHangs.listChild.list;
        ...
        else if(this.props.khachLeTraHangFormChild.mode === 'edit'){
            // const total_money_before_discount = this.props.internalStockDeliveries.selectedDetail.total_money_before_discount;
            const removeObj = this.props.khachLeTraHangs.listChild.list[this.state.keyRowSelected];
            const discount_remove = 0;//removeObj.thanh_tien_tra;
            const vat_tax_money_remove = 0;//removeObj.vat_tax_money;

            const list_remain = this._removeByKey(this.props.khachLeTraHangs.listChild.list, this.state.keyRowSelected);
...
//                this.props.formClear();
    ...
    _calculatorMoneyParentForm(money_add, mode, vat_money_add){
        var total_money_before_discount = this.props.khachLeTraHangForm.values.total_money_before_discount ? this.props.khachLeTraHangForm.values.total_money_before_discount : 0;
        var delivery_total_money = this.props.khachLeTraHangForm.values.delivery_total_money ? this.props.khachLeTraHangForm.values.delivery_total_money : 0;
        var total_discount_money = this.props.khachLeTraHangForm.values.total_discount_money ? this.props.khachLeTraHangForm.values.total_discount_money : 0;
        var discount_rate = this.props.khachLeTraHangForm.values.discount_rate ? this.props.khachLeTraHangForm.values.discount_rate : 0;
        var total_vat_money = this.props.khachLeTraHangForm.values.total_vat_money ? this.props.khachLeTraHangForm.values.total_vat_money : 0; // total vat
        ...
        this.props.khachLeTraHangFormChange('total_money_before_discount', total_money_before_discount_added);
        this.props.khachLeTraHangFormChange('delivery_total_money', delivery_total_money_added);
        this.props.khachLeTraHangFormChange('total_discount_money', total_discount_money_added);
        this.props.khachLeTraHangFormChange('total_vat_money', total_vat_money_added);
    }
    ...
    _onChangeChildField(field, value){
        ...
        switch(field){
            case 'so_luong_tra': {              
                if(Check.CheckEmpty(value) || parseInt(value) < 1){
                ...
                else if((parseInt(this.props.khachLeTraHangFormChild.values.quantity) < (parseInt(value) + parseInt(this.state.returnMedicineCount)))){
                    error =  'application.validation.spaceErrors';
                    Helper.warningAlert('Số lượng nhập phải bé hơn hoặc bằng số lượng bán');
                }
                else{
                    error = '';
                    let vatTotalTaxMoney = Helper.getVATQuanlityItem(this.props.khachLeTraHangFormChild.values.input_price, this.props.khachLeTraHangFormChild.values.tax_level_id, value);
                    let totalPrice = parseFloat(value) * parseFloat(this.props.khachLeTraHangFormChild.values.input_price);
                    ...
                    var so_luong_con_lai = parseInt(this.props.khachLeTraHangFormChild.values.quantity) - parseInt(this.state.returnMedicineCount);
                    this.props.formChange('so_luong_con_lai', so_luong_con_lai);
                }
                break;
            }
    ...
    _onValidationSubmit(){
        const {so_luong_tra} = this.props.khachLeTraHangFormChild.values;
        this._onChangeChildField('so_luong_tra', so_luong_tra);

        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const errors = this.props.khachLeTraHangFormChild.errors;
                let valid = true;
                        ...
                        <div className="form-body">                        
                            <div className="table-scrollable">
                                <table className="table fixed-header table-bordered" id="search_bill_result_list">
                                    ...
                                    {
                                        (this.props.khachLeTraHangs.listChild === 'undefined' || this.props.khachLeTraHangs.listChild.list === 'undefined' || this.props.khachLeTraHangs.listChild.list.length == 0)
                                        ?
                                        (    <tr>
                                        ...
                                        :
                                        (
                                        this.props.khachLeTraHangs.listChild.list.map((child, key) => {
                                            const selected = (this.props.khachLeTraHangs.listChild.selectedDetail.id === child.id) ? 'row-active': '';
                            ...
                            <div className="row">
                                ...
                                <div className="col-sm-1">
                                        <div className={this.props.khachLeTraHangFormChild.errors.so_luong_tra?'form-group has-error':'form-group'}>
                                        <label>SL trả</label>
                                        <input type="number" className="form-control text-right" value={selectedDetail.so_luong_tra} min="0" onChange={event => this._onChangeChildField('so_luong_tra', event.target.value)} disabled={this.props.khachLeTraHangForm.disabled}/>
                                        <span className="help-block" style={{display: this.props.khachLeTraHangFormChild.errors.so_luong_tra ? 'block': 'none'}}>
                                            <Translate value={this.props.khachLeTraHangFormChild.errors.so_luong_tra}/>
                                        </span>
                                        </div>
                                    </div>
                                <div className="col-md-2">
                                    <div className={this.props.khachLeTraHangFormChild.errors.thanh_tien_tra?'form-group has-error':'form-group'}>
                                        <label>Thành tiền</label>
                                        <input type="text" className="form-control text-right" value={Numeral(selectedDetail.thanh_tien_tra).format('0,0')} readOnly/>
                                        <span className="help-block" style={{display: this.props.khachLeTraHangFormChild.errors.thanh_tien_tra ? 'block': 'none'}}>
                                            <Translate value={this.props.khachLeTraHangFormChild.errors.thanh_tien_tra}/>
                                        </span>
                                    </div>
                                </div>
                                <div className="col-sm-2">
                                        ...
                                        <label>&nbsp;&nbsp;&nbsp;</label>
                                        <div>
                                            <button className="btn green btn-block" type="button" onClick={this._bindAccept.bind(this)} disabled={this.props.khachLeTraHangForm.disabled}>Cập nhật</button>
                                        </div>
                                    </div>
                                    </div>
                            </div>
...
const mapDispatchToProps = (dispatch) => {
    return Redux.bindActionCreators({
        ...
        ...KhachLeTraHangFormChildActions,
