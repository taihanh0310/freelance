...
export const loadList = (date_from, date_to) => {
	...
	return new Promise((resolve, reject) => {
            setTimeout(() => {
		axios.get(`${Config.API_URL}khach-le-tra-hang?date_from=${date_from}&date_to=${date_to}`)
		.then(response => {
                    resolve();
			const list = response.data.data;
			dispatch({type: KHACH_LE_TRA_HANG_CHANGE_LIST, payload: {listRoot: list, list}});
                    });
		}, Config.TIMEOUT);
            });
    };
...
export const onSearchBill = (values) => {
	...
	return new Promise((resolve, reject) => {
            setTimeout(() => {
                axios.post(`${Config.API_URL}retailer/search-medicine`, {values})
		.then(response => {
                    resolve();
			const list = response.data.data;
                        
			dispatch({type: KHACH_LE_TRA_HANG_CHANGE_SEARCH_RESULT_LIST, payload: list});
                    });
		}, Config.TIMEOUT);
            });
    };
...
export const onClickSearchResultBill = (code) => {
	return (dispatch, getState) => {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				axios.get(`${Config.API_URL}retailer/code/${code}`)
				.then(response => {
					resolve();
					let detail = response.data.data;
                                        if(detail == 'undefined' || detail.length == 0){
                                            ...
                                            dispatch({type: KHACH_LE_TRA_HANG_CHANGE_SELECTED_DETAIL, payload: detail});
                                            dispatch({type: KHACH_LE_TRA_HANG_CHANGE_LIST_CHILD, payload: detail.retailer_details});
                                        }
				}).catch(function(err){
                                    toastr.error('Không tìm thấy hóa đơn bán sỉ', 'Cảnh báo');
                                });
...
export const chooseSelectedDetail = (selectedDetail) => {
	return (dispatch, getState) => {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				axios.get(`${Config.API_URL}khach-le-tra-hang/${selectedDetail.id}`)
				.then(response => {
					resolve();
...
export const clearSelectedDetail = () => {
	return {
		type: KHACH_LE_TRA_HANG_CLEAR_SELECTED_DETAIL,
		payload: false
	};
};
...
export const chooseListChildSelectedDetail = (selectedDetail) => {
	return {
		type: KHACH_LE_TRA_HANG_CHANGE_LIST_CHILD_SELECTED_DETAIL,
...
export const clearListChildSelectedDetail = () => {
	return {
		type: KHACH_LE_TRA_HANG_CLEAR_LIST_CHILD_SELECTED_DETAIL,
		payload: false
	};
};
...
export const pushListChild = (detail) => {
	return {
		type: KHACH_LE_TRA_HANG_PUSH_LIST_CHILD,
		payload: detail
	};
};
...
export const clearListChild = () => {
	return {
		type: KHACH_LE_TRA_HANG_CLEAR_LIST_CHILD,
		payload: false
	};
};
...
export const changeListChild = (list) => {
	return {
		type: KHACH_LE_TRA_HANG_CHANGE_LIST_CHILD,
		payload: list
	};
};
...
export const changeSearchList = (field, value) => {
    return {
        type: KHACH_LE_TRA_HANG_CHANGE_SEARCH_LIST,
        payload: {field, value}
    };
};
