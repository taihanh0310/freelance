...
import * as RetailerFormActions from 'modules/retailer/actions/form';
import {changeList as medicineChangeList} from 'modules/medicine/actions/list';
import {formClear as formChildClear} from 'modules/retailer/actions/formChild';

class RetailerParentSearchResult extends React.Component {
	constructor(){
        super();
        this.state = {
    ...
    _onNextRow() {
        if(this.props.retailers.selectedDetail.id) {
            if (this.state.selectedKey < this.props.retailers.list.length - 1) {
                const nextKey = this.state.selectedKey+1;
                const selectedDetail = this.props.retailers.list[nextKey];
    ...
    _onPreviousRow() {
        if (this.props.retailers.selectedDetail.id) {
            const retailers = this.props.retailers.list;
            if (this.state.selectedKey > 0) {
                const previousKey = this.state.selectedKey - 1;
                const selectedDetail = retailers[previousKey];
                this.props.chooseSelectedDetail(selectedDetail);
    ...
    _onClickRow(retailer, key){
        ...
        this.props.chooseSelectedDetail(retailer);
        this.props.formChangeMode('edit');
        setTimeout(() => {
            axios.get(`${Config.API_URL}pharmacy-warehouse/${retailer.from_drug_store_id}/medicine`)
            .then(response => {
                    ...
                    <div className="table-scrollable">
                        <table className="table fixed-header table-bordered" id="search_bill_result_list">
                            ...
                            <tbody id="list-container">
                            {
                                ...
                                :
                                (
                                    this.props.retailers.list.map((retailer, key) => {
                                        const selected = (this.props.retailers.selectedDetail.id === retailer.id) ? 'row-active': '';
                                        return (
                                            <tr key={key} className={selected} onClick={this._onClickRow.bind(this, retailer, key)}>
                                                <td>{retailer.code}</td>
                                                <td>{Helper.formatStringToDate(retailer.delivery_date)}</td>
                                            </tr>
                                        );
                                    })
                                )
...
module.exports = ReactRedux.connect(mapStateToProps, mapDispatchToProps)(RetailerParentSearchResult);