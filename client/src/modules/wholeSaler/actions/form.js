...
export const formValidation = (field, value) => {
	return {
		type: WHOLE_SALER_FORM_VALIDATION,
		payload: {field, value}
	};
};
...
export const formDelete = (id) => {
	return (dispatch) => {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				axios.delete(`${Config.API_URL}whole-saler/${id}`)
				.then(response => {
					resolve();
