import {
	DRUG_STORE_FORM_CHANGE,
	DRUG_STORE_FORM_VALIDATION,
	DRUG_STORE_FORM_CLEAR,
	DRUG_STORE_FORM_CHANGE_MODE
} from '../types';
...
export const formValidation = (field, value) => {
	return {
		type: DRUG_STORE_FORM_VALIDATION,
		payload: {field, value}
	};
};
...
export const formChangeMode = (mode) => {
	return {
		type: DRUG_STORE_FORM_CHANGE_MODE,
		payload: mode
	};
};
...
export const formAdd = (values) => {
	return (dispatch) => {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				axios.post(`${Config.API_URL}drug-stores`, values)
				.then(response => {
...
export const formDelete = (id) => {
	return (dispatch) => {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				axios.delete(`${Config.API_URL}drug-stores/${id}`)
				.then(response => {
					resolve();
...
export const formEdit = (values) => {
	return (dispatch) => {
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				axios.put(`${Config.API_URL}drug-stores/${values.id}`, values)
				.then(response => {
					resolve();
					dispatch({type: 'EDIT'});
				})
