...
class Buttons extends React.Component{
    ...
    _onPrintReview(){
        ...
        axios.post(`${Config.API_URL}thong-ke-thuoc-dang-duoc-ban/pdfExport`, {values}, {
        ...
        .then((response) => {
            ...
            const fileName = `thuoc_dang_duoc_ban_${date}.pdf`;
    ...
    _onExcelExport() {
        let values = $.extend({}, this.props.thuocDangDuocBans.search);
        axios.post(`${Config.API_URL}thong-ke-thuoc-dang-duoc-ban/excelExport`, {values}, {
            responseType: 'arraybuffer'
        })
                .then((response) => {
                    let downloadLink = document.createElement('a');
                    ...
                    const date = Moment(new Date()).format('YYYY.MM.DD_HH.mm.ss');
                    const fileName = `danh_sach_thuoc_dang_duoc_ban_${date}.xlsx`;
                    downloadLink.download = fileName;
